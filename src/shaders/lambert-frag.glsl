...
const float PI=3.1415926535928;
...
float FBM3D(vec2 pos, float u_Time){
    int time_residue=int(floor(u_Time*240.0f))%60;
    float amp=0.5f*sin(float(time_residue)/60.0f*2.0f*PI);
    float total=0.0f;
    float persistance=0.04f;
...
void main()
{
    // Material base color (before shading)
        ...
        out_Col = vec4(diffuseColor.rgb * lightIntensity, diffuseColor.a*FBM3D(fs_uvs,u_Time));
